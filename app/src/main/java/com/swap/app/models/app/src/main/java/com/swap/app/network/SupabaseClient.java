...
public class SupabaseClient {
    ...
    public void get(String table, String params, ApiCallback callback) {
        exec(baseRest(table, params).get().build(), callback);
    }

    public void post(String table, String json, ApiCallback callback) {
        exec(baseRest(table, null).post(RequestBody.create(json, MediaType.parse("application/json"))).build(), callback);
    }

    public void patch(String table, String params, String json, ApiCallback callback) {
        exec(baseRest(table, params).patch(RequestBody.create(json, MediaType.parse("application/json"))).build(), callback);
    }

    public void delete(String table, String params, ApiCallback callback) {
        exec(baseRest(table, params).delete().build(), callback);
    }

    private void exec(Request request, ApiCallback callback) {
        httpClient.newCall(request).enqueue(new Callback() {
            @Override public void onFailure(Call call, IOException e) { Log.e(TAG, e.getMessage()); callback.onError(e.getMessage()); }
            @Override public void onResponse(Call call, Response response) throws IOException {
                String body = response.body() != null ? response.body().string() : "";
                if (response.isSuccessful()) callback.onSuccess(body);
                else { Log.e(TAG, "HTTP " + response.code() + ": " + body); callback.onError("HTTP " + response.code()); }
            }
        });
    }
}
